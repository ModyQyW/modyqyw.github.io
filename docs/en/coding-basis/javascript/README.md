...
闭包是引用了另一个函数作用域内变量的函数，通常在嵌套函数内实现。闭包使得这些变量不会因为垃圾回收机制而被自动回收，某种意义上说，闭包是实现私有属性和私有方法的一种手段。
...
反过来说，由于闭包使用到的变量不会被自动回收，所以很容易导致内存泄漏，还可能污染全局变量。应该要尽可能地克制使用闭包，使用闭包时应该要在退出时清理（赋值为 `null` 即可）。
...
在这里有一个隐蔽的闭包，那就是 `setTimeout` 里面的回调方法。事实上，回调方法都可以视作闭包。
