...
[Chrome](https://www.google.com/intl/en_us/chrome/) 仍然是我的第一选择。为了避免 Google 重定向，可以访问一下 [google.com/ncr](google.com/ncr)。
...
随后使用 Homebrew 安装 [zsh](https://formulae.brew.sh/formula/zsh#default)、[zsh-autosuggestions](https://github.com/zsh-users/zsh-autosuggestions)、[zsh-fast-syntax-highlighting](https://github.com/zdharma-continuum/fast-syntax-highlighting)，另外根据指引安装 [oh-my-zsh](https://ohmyz.sh/#install)，配合起来一起使用。
...
```shell
plugins=(git vscode web-search z)
...
为了用模拟器调试移动应用，一些额外的开发工具，比如 JDK、[Android Studio](https://developer.android.com/studio) 和 [XCode](https://developer.apple.com/xcode/) 等也必不可少。

```shell
brew install openjdk cocoapods gradle watchman
